...
## Version 2.2
Переменная isAuth вынесена в отдельный файл UserStore.jsx. В нем настроена работа mobx для проверки изменений этой переменной. В AppRouter.jsx можно вывести переменную с помощью console.log(user) , в браузере через "Просмотреть код" можно развернуть объект UserStore с нужной переменной isAuth. Это - глобальное хранилище переменных.

## Version 2.3
Создана навигационная панель. Observer из mobx-react-lite позволяет отслеживать изменения состояний. В зависимости от значения переменной isAuth NavBar имеет разный функционал.
onClick={() => user.setIsAuth(true)} - проба работы с переменной isAuth. Задает значение истины при нажатии на кнопку "Авторизация".
